        ...
        if "loss_mode" in input:
            if "sup" in input["loss_mode"]:
                ...
                output["loss"] += input["lam"] * loss_fn(mix_output, input["mix_target"][:, 0].detach()) + (
                    1 - input["lam"]
                ) * loss_fn(mix_output, input["mix_target"][:, 1].detach())
            else:
...
def wresnet28x2(momentum: float = None, track: bool = False) -> WideResNet:
...
def wresnet28x8(momentum: float = None, track: bool = False) -> WideResNet:
    data_shape = cfg["data_shape"]
    target_size = cfg["target_size"]
...
def wresnet37x2(momentum: float = None, track: bool = False) -> WideResNet:
    data_shape = cfg["data_shape"]
    target_size = cfg["target_size"]
